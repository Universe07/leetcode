...
// Go through each pair of x coords in the hash map. Ignore x coords that have
// less than 2 y-s. Intersect two sets to get common y coords for this pair of x
// coords. Since y coords are sorted, you only need to calculate the area for
// the y[k] - y[k - 1] pairs.

// Complexity Analysis
// Runtime: O(n* n).We check each x coordinate agains all other x
// coordinates.The set intersection is linear. Memory : O(n).We store all points
// in our sets.
